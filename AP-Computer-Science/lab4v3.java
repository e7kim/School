...
//		}
		...
		do
		{
			...
			if(choice==1)
			{
				...
				System.out.println("The shot comes out at " + shootLaser(board,x,y,direction) + ".");
				shots++;
	...
	public static void setMirrors(char[][] board)
	{
	...
	public static int shootLaser(char[][] board, int x, int y, int direction)
	{
		if(y==-1 && direction==3)
			return x;
		...
		{
			if(direction==1)
				return shootLaser(board, x+1, y, 2);
			else if(direction==2)
				return shootLaser(board, x, y+1, 1);
			else if(direction==3)
				return shootLaser(board, x-1, y, 4);
			else if(direction==4)
				return shootLaser(board, x, y-1, 3);
		}
